#  Adapted from Argilla - https://github.com/argilla-io/argilla/
#  Original Copyright notice: Copyright 2021-present, the Recognai S.L. team.
#
...
def require_version(requirement: str, func_name: Optional[str] = None) -> None:
    """
    Perform a runtime check of the dependency versions, using the exact same syntax used by pip. The installed module version comes from the *site-packages* dir via *importlib_metadata*.
    ...
    if re.match(r"^[\w_\-\d]+$", requirement):
        ...
        for w in want_range:
            ...
            if op not in ops:
                raise ValueError(
                    f"{requirement}: need one of {list(ops.keys())}, but got {op!r}."
                )
    ...
    if package == "python":
        ...
        for op, want_version in wanted.items():
            _compare_versions(
                op, got_version, want_version, requirement, package, func_name=func_name
            )
        return
